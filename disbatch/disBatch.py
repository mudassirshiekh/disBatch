...
warnings.formatwarning = (
    lambda msg, cat, *args, **kwargs: f'{cat.__name__}: {msg}\n'
    ...
    def retireNodeList(self, nodeList, retList):
        ...
        if self.retireCmd:
            ...
            logger.info(
                ...
                [
                    (v, env.get(v, '<NOT SET>'))
                    for v in ['DRIVER_NODE', 'NODES', 'RETCODES', 'ACTIVE', 'RETIRED']
                ],
            )
            try:
                capture = SUB.run(
                ...
                if capture.stdout:
                    logger.info(
                        'Retirement stdout: "%s"', capture.stdout.decode('utf-8')
                    )
                if capture.stderr:
                    logger.info(
                        'Retirement stderr: "%s"', capture.stderr.decode('utf-8')
                    )
        else:
...
def nl2flat(nl):
    return SUB.check_output(
        ['scontrol', 'show', 'hostnames', nl], universal_newlines=True
    ).splitlines()
...
class SlurmContext(BatchContext):
    ...
    def __init__(self, dbInfo, rank, args):
        ...
        if self.tasksPerNode:
            ...
            if args.fill:
                if (
                    ...
                    or 'GPU_DEVICE_ORDINAL' in os.environ
                ):
                    self.for_log.append(
                        (
                            'Fill requested, but GPUs detected. Ignoring request.',
                            logging.WARNING,
                        )
                    )
                else:
            ...
            if do_fill:
                self.for_log.append(
                    ('Fill requested, ignoring SLURM_TASKS_PER_NODE.', logging.WARNING)
                )
                cylinders = [int(jcpn // self.cpusPerTask) for jcpn in jcpnl]
                self.for_log.append(
                    (
                        f'Cores per node: {jcpnl} /({self.cpusPerTask} cores per task) -> {cylinders}',
                        logging.INFO,
                    )
                )
            else:
                ...
                cylinders = [
                    min(stpn, int(jcpn // self.cpusPerTask))
                    for stpn, jcpn in zip(self.stpnl, jcpnl)
                ]
                self.for_log.append(
                    (
        ...
        if cores_per_cylinder is None:
            cores_per_cylinder = [
                jcpn // c if c else jcpn for jcpn, c in zip(jcpnl, cylinders)
            ]
        ...
        if opt_file:
            ...
            opts = [
                'SLURM_CPU_BIND=' + os.getenv('SLURM_CPU_BIND', 'cores'),
                'SLURM_MPI_TYPE=none',
            ]
        if opts:
        ...
        self.throttle_thread = Thread(
            target=self.__retirementThrottle__, name='throttle', daemon=True
        )
    ...
    def engine_start(tag):
        ...
        to_server_template, from_server_template = (
            f'/tmp/{tag}_to_task_server_%d',
            f'/tmp/{tag}_from_task_server_%d',
        )
        ...
        to_server, from_server = (
            to_server_template % slurm_local_rank,
            from_server_template % slurm_local_rank,
        )
        ...
        while True:
            ...
            sh_err, sh_out = (
                f'/tmp/{tag}_{slurm_local_rank}_{seq}_err',
                f'/tmp/{tag}_{slurm_local_rank}_{seq}_out',
            )
            ...
            p = SUB.Popen(
                ...
                env=env,
                stdin=None,
                stdout=open(sh_out, 'wb'),
                stderr=open(sh_err, 'wb'),
            )
            r = p.wait()
    ...
    def launchNode(self, n):
        ...
        if self.stpnl[nx] != tasks:
            logging.warning(
                f'SLURM believes tasks should be {self.stpnl[nx]}, attempting to run {tasks}.'
            )
    ...
    def run_task(self, target_cylinder, task_info, env):
        incoming, outgoing = (
            self.incoming_pipes[target_cylinder],
            self.outgoing_pipes[target_cylinder],
        )
        pickle.dump((task_info.taskId, task_info.taskCmd, env), outgoing)
        outgoing.flush()
        sh_err, sh_out = pickle.load(incoming)
        p = SUB.Popen(
            ['/bin/bash', '-c', f'cat < {sh_out} & cat < {sh_err} >&2'],
            stdin=None,
            stdout=SUB.PIPE,
            stderr=SUB.PIPE,
        )
...
class SSHContext(BatchContext):
    def __init__(self, dbInfo, rank, args):
        ...
        nodelist = (
            args.ssh_node if args.ssh_node else os.getenv('DISBATCH_SSH_NODELIST')
        )
        ...
        for p in nodelist.split(','):
            ...
            if n in node_set:
                ...
                self.for_log.append(
                    (
                        f'Repeated {n} so summing cores {core_count[nx]} + {e}',
                        logging.WARN,
                    )
                )
                core_count[nx] += e
        ...
        if self.tasksPerNode:
            ...
            self.for_log.append(
                (
                    f'Tasks per node: {cylinders}, using {self.cpusPerTask} cores per task.',
                    logging.INFO,
                )
            )
        cores_per_cylinder = [
...
class TaskInfo:
    ...
    def __init__(
        ...
        (
            ...
            self.taskKey,
            self.bKey,
        ) = taskId, taskStreamIndex, taskRepIndex, taskCmd, taskKey, bKey
        ...
        if len(args) == 1 and len(kwargs) == 0 and isinstance(args[0], str):
            ...
            try:
                if args[0] == TaskReport.header:
                    ...
                    fx = partial(
                        TaskReport.find_field,
                        args[0].split('\t', TaskReport.num_fields - 1),
                    )
                    kind = 'N'
                    flags = fx('Flags')
                    if flags[3] in 'PSZ':
...
class TaskProcess:
    ...
    def poll(self):
        ...
        logger.info(
            'Task generating command has exited: %s %d.', repr(self.command), self.r
        )
    ...
    def peEndListTasks():
        for when in ['START', 'STOP']:
            yield TaskInfo(
                ...
                -1,
                b'#ENDLIST',
                '.per engine %s %d' % (when, peCounters[when]),
                kind='P',
            )
    ...
    while OK:
        ...
        for t in t.splitlines():
            ...
            if m:
                ...
                yield TaskInfo(
                    taskCounter, tsx, -1, t, '.barrier', kind=kind, bKey=bKey
                )
                taskCounter += 1
                continue
    ...
    while True:
        ...
        if t.kind == 'N':
            ...
            if s and (not retry or s.skipInfo.returncode == 0):
                ...
                if s != t:
                    msg = (
                        'Recovery status file task mismatch %s:\n'
                        + repr(s)
                        + '\n'
                        + repr(t)
                    )
...
class Driver(Thread):
    def __init__(self, kvs, db_info, tasks, trackResults=None):
        ...
        self.kvs.put(
            '.common env',
            {
                'DISBATCH_JOBID': os.path.split(str(self.db_info.uniqueId))[-1],
                'DISBATCH_NAMETASKS': self.db_info.name,
            },
        )
    ...
    def sendNotification(self):
        try:
            ...
            msg['Subject'] = '%s has completed %d tasks' % (
                self.db_info.uniqueId,
                self.finished,
            )
            if self.failed:
        ...
        self.kvs.put(
            ...
            json.dumps(
                ...
                default=lambda o: dict(
                    [t for t in o.__dict__.items() if t[0] != 'kvs']
                ),
            ),
            b'JSON',
        )
    ...
    class EngineProxy:
        def __init__(self, rank, cRank, hostname, pid, start, kvs):
            self.rank, self.cRank, self.hostname, self.pid, self.kvs = (
                rank,
                cRank,
                hostname,
                pid,
            ...
            self.active, self.assigned, self.cylinders, self.failed, self.finished = (
                0,
                0,
                {},
                0,
    ...
    def run(self):
        ...
        cRank2taskCount, cylKey2eRank, enginesDone, finishedTasks, hbFails = (
            DD(int),
            {},
            False,
            {},
        ...
        while not (self.tasksDone and enginesDone):
            ...
            if msg == 'clearing barriers':
                ...
                if e.status != 'running' or notifiedAllDone:
                    ...
                    logger.info(
                        'Engine %s (%s), "%s" available', engineRank, e.hostname, ckey
                    )
                ...
                if self.tasksDone:
                    ...
                    for e in self.engines.values():
                        if e.status != 'stopped':
                            if (now - e.last) > NoPulse:
                                ...
                                logger.debug(
                                    'Engine %d in ICU (%.1f)', e.rank, (now - e.last)
                                )
                                enginesDone = False
                else:
                    for tinfo, ckey, start, ts in outstanding.values():
                        if now - ts > NoPulse:
                            ...
                            if (
                                tinfo.taskId not in hbFails
                            ):  # Guard against a pile up of heart beat msgs.
                                hbFails.add(tinfo.taskId)
                                self.kvs.put(
                                    '.controller',
                                    ('task hb fail', (tinfo, ckey, start, ts)),
                                )
                ...
                self.engines[rank] = self.EngineProxy(
                    rank, cRank, hn, pid, start, self.kvs
                )
                ...
                enginesDone = not [
                    e for e in self.engines.values() if e.status != 'stopped'
                ]
                ...
                self.barriers.append(
                    TaskReport(
                        TaskInfo(o, -1, -1, b'#NO MORE TASKS BARRIER', None, kind='D'),
                        start=time.time(),
                    )
                )
                ...
                if msg == 'task skipped':
                    ...
                    tReport = TaskReport(
                        tinfo, self.engines[engineRank].hostname, -1, -100, start, last
                    )
                    hbFail = True
                else:
                ...
                if not zombie:
                    ...
                    if rc:
                        ...
                        assert (
                            self.barriers == []
                            or tinfo.taskId < self.barriers[0].taskInfo.taskId
                        )
                    ...
                    if self.trackResults:
                        ...
                        self.kvs.put(
                            self.trackResults + b' done tasks', str(tinfo.taskId), False
                        )
                    if (
                        self.db_info.args.mailTo
                        and self.finished % self.db_info.args.mailFreq == 0
                    ):
            ...
            if self.barriers:
                ...
                for x in range(retired + 1, self.barriers[0].taskInfo.taskId):
                    if x not in finishedTasks:
                        retired = x - 1
                        logger.debug(
                            'Barrier waiting for %d (%d)',
                            x,
                            self.barriers[0].taskInfo.taskId,
                        )
                        break
                else:
    ...
    class Cylinder(Thread):
        def __init__(
            ...
            (
                ...
                self.cylinderRank,
                self.fetchTask,
            ) = context, hbQueue, engineRank, cylinderRank, fetchTask
            self.localEnv = env.copy()
            self.localEnv['DISBATCH_CORES_PER_TASK'] = str(
                self.context.cores_per_cylinder[self.context.nodeId]
            )
            logger.info(
                'Cylinder %d initializing, %s cores',
                self.cylinderRank,
                self.localEnv['DISBATCH_CORES_PER_TASK'],
            )
        ...
        def run(self):
            ...
            try:
                ...
                killPatiently(
                    self.taskProc, 'cylinder %d subproc' % self.cylinderRank, 2
                )
        ...
        def main(self):
            ...
            logger.info(
                'Cylinder %d firing, %d, %d.', self.cylinderRank, self.pid, self.pgid
            )
            if self.cylinderRank != -1:
                self.kvs.put(
                    ...
                    (
                        'cylinder available',
                        (self.engineRank, self.pid, self.pgid, self.fetchTask.keyTemp),
                    ),
                )

            (
                self.localEnv['DISBATCH_CYLINDER_RANK'],
                self.localEnv['DISBATCH_ENGINE_RANK'],
            ) = str(self.cylinderRank), str(self.engineRank)
            while 1:
                ...
                if msg == 'stop':
                    logger.info(
                        'Cylinder %d received %s, exiting.', self.cylinderRank, msg
                    )
                    if self.cylinderRank != -1:
                        self.kvs.put(
                            '.controller',
                            (
                                'cylinder stopped',
                                (self.engineRank, self.fetchTask.keyTemp),
                            ),
                        )
                ...
                t0 = time.time()
                try:
                    self.taskProc = self.context.run_task(
                        self.cylinderRank, ti, self.localEnv
                    )
                    ...
                    while True:
                        ...
                        if ct >= PulseTime:
                            ...
                            self.hbQueue.put(
                                (
                                    self.engineRank,
                                    -1 if self.cylinderRank == -1 else ti.taskId,
                                )
                            )
                            ct = 0.0
    ...
    def __init__(self, kvs, context, rank):
        ...
        for v in context.envres:
            ...
            if e:
                ...
                if len(L) < cylinders:
                    logger.error(
                        'Requested envres variable "%s" has too few values, decreasing cylinders to match: %s',
                        v,
                        e,
                    )
        ...
        self.kvs.put(
            ...
            (
                'engine started',
                (self.rank, context.rank, myHostname, myPid, time.time()),
            ),
        )
        ...
        logger.info(
            'Engine %d running normal tasks, %d-way concurrency', self.rank, cylinders
        )
        self.cylinders = [
            self.Cylinder(
                context,
                ...
                x,
                self.FetchTask(
                    '.cylinder %d %d' % (self.rank, x),
                    constantKeyGen,
                    kvsstcp.KVSClient.get,
                ),
            )
...
def contextArgs(argp):
    ...
    argp.add_argument(
        '--fill',
        action='store_true',
        help='Try to use extra cores if allocated cores exceeds requested cores.',
    )
    argp.add_argument(
        '-g',
        '--gpu',
        action='store_true',
        help='Use assigned GPU resources [DEPRECATED]',
    )
    argp.add_argument(
        '--no-retire',
        dest='retire_cmd',
    ...
    argp.add_argument(
        ...
        type=str,
        metavar='COMMAND',
        help='Label for this context. Should be unique.',
    )
    argp.add_argument(
        '--retire-cmd',
        type=str,
...
def main(kvsq=None):
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--engine':
        ...
        argp.add_argument(
            '--engine', action='store_true', help='Run in execution engine mode.'
        )
        argp.add_argument('--method', type=str, help='Context specific start up.')
        argp.add_argument(
            '--tag',
            type=str,
            help='Random indentifier shared by sibling task servers and their engine.',
        )
        ...
        if args.method:
            context.__getattribute__(args.method.split('.', 1)[-1] + '_state')(
                method_state
            )
        ...
        lconf['filename'] = '%s_%s_%s_engine_%d.log' % (
            dbInfo.uniqueId,
            context.label,
            args.node,
            rank,
        ...
        logger.info(
            'Starting engine %s (%d) on %s (%d) in %s.',
            context.node,
            rank,
            myHostname,
            myPid,
        ...
        logger.info(
            'Remaining processes:\n'
            + SUB.check_output(['ps', 'fuhx', '--cols', '1000']).decode(
                'utf-8', 'ignore'
            )
        )
        sys.exit(0)
        ...
        if nogo:
            logger.warning(
                'At least one engine lacks enough cylinders to run tasks (%r).'
                % ([context.nodes[x] for x in nogo])
            )
        ...
        if args.gpu:
            print(
                '-g/--gpu is deprecated. It is no longer needed and will be ignored.',
                file=sys.stderr,
            )
        context.envres = (
            ','.join(args.env_resource).split(',') if args.env_resource else []
        )
        ...
        argp = argparse.ArgumentParser(
            description='Use batch resources to process a file of tasks, one task per line.'
        )
        argp.add_argument(
            '-e',
            '--exit-code',
        ...
        argp.add_argument(
            '--force-resume',
            action='store_true',
            help='With -r, proceed even if task commands/lines are different.',
        )
        argp.add_argument(
            '--kvsserver',
            nargs='?',
        ...
        argp.add_argument(
            ...
            metavar='ADDR',
            default=None,
            help='Mail address for task completion notification(s).',
        )
        argp.add_argument(
            '-p',
            '--prefix',
        ...
        argp.add_argument(
            '--status-header',
            action='store_true',
            help='Add header line to status file.',
        )
        argp.add_argument(
            '--use-address',
            default=None,
            metavar='HOST:PORT',
            help='Specify hostname and port to use for this run.',
        )
        argp.add_argument(
            '-w', '--web', action='store_true', help='Enable web interface.'
        )
        ...
        source.add_argument(
            ...
            default=False,
            metavar='HOST:PORT',
            help='Tasks will come from the KVS server.',
        )
        source.add_argument(
            'taskfile',
            nargs='?',
        ...
        if args.startup_only and (
            args.env_resource or args.retire_cmd or args.ssh_node
        ):
            argp.print_help()
            sys.exit(1)
        ...
        if not args.kvsserver:
            ...
            print(
                'Cannot use different --kvsserver and --taskservers.', file=sys.stderr
            )
            sys.exit(1)
        ...
        if os.path.isdir(rp):
            uniqueId = rp + '/%s_disBatch_%s_%03d' % (
                tfn,
        ...
        if args.resume_from:
            tasks = statusTaskFilter(
                tasks,
                parseStatusFiles(*args.resume_from),
                args.retry,
                args.force_resume,
            )
        ...
        with open(DbUtilPath, 'w', opener=partial(os.open, mode=0o700)) as fd:
            fd.write(
                dbutil_template.format(
                    DisBatchPython=DisBatchPython,
                    DbUtilPath=DbUtilPath,
                    kvsserver=kvsserver,
                    uniqueId=uniqueId,
                )
            )
        ...
        if subContext and subContext.returncode:
            print(
                'Some engine processes failed -- please check the logs', file=sys.stderr
            )
            sys.exit(1)

        if args.exit_code and driver.failed:
            print(
                'Some tasks failed with non-zero exit codes -- please check the logs',
                file=sys.stderr,
            )
            sys.exit(1)
